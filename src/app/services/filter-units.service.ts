...
type HOUR_INDEXS = 'morning' | 'afternoon' | 'night';
...
export class FilterUnitsService {
  ...
  filterUnits(unit: Location, open_hour: string, close_hour: string) {
    ...
    let close_hour_filter = parseInt(open_hour, 10);
  ...
  filter(dataSource: Location[], showClosed: boolean, hour: string) {
    let intermediateResults = dataSource;

    if (!showClosed) {
      intermediateResults = dataSource.filter(location => location.opened === true);
    }

    if (hour) {
      const OPEN_HOUR = OPENING_HOURS[hour as HOUR_INDEXS].first;
      const CLOSE_HOUR = OPENING_HOURS[hour as HOUR_INDEXS].last;
